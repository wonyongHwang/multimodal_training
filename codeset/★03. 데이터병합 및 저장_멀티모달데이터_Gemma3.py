...
def loadBaseModel(cfg):
    """베이스 모델과 프로세서를 4-bit 양자화로 로드하여 반환"""
...
def uploadToHub(mergedModel, processor, mergedModelRepo):
    """병합된 모델과 프로세서를 HuggingFace Hub에 업로드"""
    try:
        print(f"Hub 업로드 중: {mergedModelRepo}")
        mergedModel.push_to_hub(mergedModelRepo)
        processor.push_to_hub(mergedModelRepo)
...
def main():
    ...
    # ===== Hub 업로드 =====
    uploadToHub(mergedModel, processor, cfg['merged_model_repo'])
